...
\caption[Comparativa de métricas]{Comparativa con modelos de referencia en métricas objetivas. Naive representa la extrapolación directa de la porción temprana sin procesamiento adicional, mientras que FiNS corresponde a la arquitectura propuesta por Steinmetz et al. (2021).}
...
La viabilidad de un modelo de acústica computacional para escenarios interactivos, como la realidad extendida, no queda determinada exclusivamente por su fidelidad termodinámica, sino también por su eficiencia paramétrica, su huella de memoria y su latencia de inferencia. En aplicaciones de audio inmersivo, estas tres magnitudes condicionan de forma directa la escalabilidad del sistema, la capacidad de despliegue en hardware de propósito general y la posibilidad de integrar la síntesis reverberante en cadenas de procesamiento en tiempo real sin degradar la experiencia perceptiva del usuario.
...
\begin{tabular}{lrrrr}
\toprule
\textbf{Modelo} & \textbf{Parámetros (M)} & \textbf{Memoria [MB]} & \textbf{Inferencia [ms]} & \textbf{RTF} \\
\midrule
DECOR -- Propuesto & 17.57 & 67.03 & 5.36 & 0.0056 \\
FiNS -- Referencia & $\approx$ 340.0 & 1300.0 & N/D & N/D \\
\bottomrule
\end{tabular}%
}
...
Desde la perspectiva del tamaño del modelo, la arquitectura propuesta, DECOR, con 17.57 millones de parámetros, se sitúa en una escala marcadamente inferior a la del modelo de referencia FiNS, cuya magnitud paramétrica alcanza aproximadamente 340 millones de parámetros y cuya huella de memoria asciende a 1.3 GB. Esta reducción equivale a un modelo casi 20 veces más ligero, lo cual confirma la eficacia del sesgo inductivo paramétrico incorporado en la formulación. En particular, al modelar explícitamente tasas de decaimiento en lugar de secuencias densas de muestras, el espacio de representación queda restringido a un subespacio físicamente interpretable, reduciéndose de forma sustancial los requisitos de almacenamiento sin comprometer la reconstrucción macroscópica del fenómeno reverberante.

En términos de latencia, el valor de 5.36 ms por inferencia, junto con un Factor de Tiempo Real de 0.0056, demuestra que el sistema sintetiza la reverberación tardía con una velocidad superior en más de 170 veces al tiempo real cuando opera bajo aceleración por hardware. Dicho resultado no constituye únicamente una mejora incremental, sino un indicador objetivo de aptitud operativa para su integración en motores de audio inmersivo, sistemas de realidad extendida y procedimientos de generación masiva de datos acústicos. En consecuencia, la combinación de baja complejidad paramétrica y latencia ultrarreducida certifica que la propuesta no solo es acústicamente consistente, sino también computacionalmente viable para contextos de despliegue exigentes.
