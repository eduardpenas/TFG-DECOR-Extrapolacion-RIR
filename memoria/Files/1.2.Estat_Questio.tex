...
Matemáticamente, el entrenamiento de una PINN para la estimación de RIR se formaliza como la minimización de una función de pérdida que combina un término de error de datos (mediciones) y un término de error físico (residuo de la ecuación de onda) \cite{krak25article}:

\begin{equation}
\mathcal{L}(\theta) = \lambda_{\mathrm{data}} \sum_{i=1}^{N} \left\| \widehat{h}(t_i; \theta) - h_{\mathrm{meas}}(t_i) \right\|^2 + \lambda_{\mathrm{physics}} \int \left| \frac{\partial^2 \widehat{h}}{\partial t^2} - c^2 \Delta \widehat{h} \right|^2 \, dt
\end{equation}              
donde $\widehat{h}(t; \theta)$ es la RIR predicha por la red con parámetros $\theta$, $h_{\mathrm{meas}}(t_i)$ son las mediciones de RIR en tiempos discretos $t_i$, y $c$ es la velocidad del sonido. Las constantes $\lambda_{\mathrm{data}}$ y $\lambda_{\mathrm{physics}}$ equilibran la importancia relativa de los términos de pérdida de datos y física.

Los resultados de \cite{krak25article}  demuestran que las PINNs pueden extrapolar con precisión los frentes de onda tempranos a partir de un número limitado de mediciones, superando a métodos tradicionales como el ISM. Sin embargo, la síntesis completa de RIRs mediante PINNs sigue siendo un desafío debido a la complejidad de las ecuaciones de onda en entornos tridimensionales y a la necesidad de un entrenamiento intensivo para converger a soluciones estables.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Figures/PINNs.png} % Asegúrate de guardar el recorte con este nombre
    \caption[Extrapolación de RIR con PINNs]{ Comparación de la reconstrucción de la RIR en términos de presión sonora a partir de un número limitado de mediciones. Se observa que la PINN logra reconstruir con precisión los frentes de onda tempranos, aunque la síntesis completa de la reverberación tardía sigue siendo un desafío. \cite{krak25article}}
    \label{fig:pinns_extrapolation}
\end{figure}

\subsection{FiNS: Filtered Noise Shaping}
En el ámbito de la estimación respuesta al impulso de la sala (RIR) a partir de voz reverberante, el modelo FiNS (Filtered Noise Shaping) \cite{Steinmetz2021FiNS} propone un enfoque basado en el aprendizaje profundo para la estimación directa en el dominio del tiempo. Este método resulta útil para aplicaciones de realidad aumentada y posproducción de audio, donde se requiere un emparejamiento acústico preciso y de alta fidelidad operando a 48 kHz.
...
Finalmente, el conjunto de las subbandas moduladas por las envolventes y la componente inicial $h_d(n)$ se combinan a través de una última capa convolucional de una dimensión (Conv1d) para producir la estimación completa y final de la respuesta al impulso de la habitación $\hat{h}(n)$.
...
\begin{figure}[H]
    ...
    \caption[Concepto de \textit{RIR completion}]{Concepto de \textit{RIR completion}: extrapolación de la reverberación tardía a partir del tramo temprano de la respuesta al impulso \cite{Lin25}.}
...
\begin{figure}[H]
    ...
    \caption[Esquema general de la arquitectura DECOR]{Esquema general de la arquitectura DECOR. El codificador procesa la cabeza de la RIR para generar un vector latente $\mathbf{z}$, que el decodificador utiliza para predecir envolventes de decaimiento aplicadas a un banco de filtros de ruido \cite{Lin25}.}
